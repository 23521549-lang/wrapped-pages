...
- Dòng đầu lịch: hai nút mũi tên đổi tháng **ngay tại chỗ**, không tải lại trang, đường dẫn vẫn ghi `?thang=YYYY-MM` để tải lại hay gửi link mở đúng tháng. "Tháng 9, 2026" ở giữa là vùng `aria-live`; dòng tổng "6 bìa, 4 lần đổi nhạc trong tháng" bên phải. Tháng sớm nhất là tháng cuốn được tạo, muộn nhất là tháng này; nút ở hai đầu tắt. `?thang` sai hay ngoài khoảng thì về tháng mặc định.
- Lịch gọn hơn Lịch hoa: cột tên làn 48px, làn **Bìa** 40px, làn **Nhạc** 18px. Làn bìa là một **xấp bìa nhỏ xòe nghiêng** (một tới ba lá, bìa của lượt đầu tiên trong ngày nằm trên cùng); làn nhạc là **chấm**, mỗi lần đổi nhạc một chấm, tối đa ba, gỡ nhạc là chấm rỗng. Không còn con số đếm. Làn trống là vòng chấm mờ. Ô ngày ở 320px vẫn rộng từ 36px (bỏ cột tên làn, thay bằng một dòng chữ như Lịch hoa). Tên ô cho trình đọc màn hình: "26 tháng 9. 6 bìa, 4 lần đổi nhạc. Hôm nay".
- **Thẻ ngày** (vùng `aria-live`): tên ngày, "7 lượt đăng: 6 bìa mới, 4 lần đổi nhạc.", và **xấp bìa lớn** bấm được ("6 bìa trong ngày", "Bấm để xem từng bìa"); rê chuột thì hai tờ sau xòe thêm. Ngày không đổi bìa thì một câu nói rõ.
- **Trình xem bìa**: bìa hiện lớn giữa màn hình trên lớp nền giấy (97%, trộn trong srgb), xếp thành chồng thẻ. Bấm, chạm hay phím → thì thẻ trên cùng lướt sang phải rồi lui xuống cuối xấp; phím ← lùi lại. Dưới bìa: "Lượt 5, trang 7 tới 8", "Bìa mới lúc 16:40", liên kết "Đọc từ trang 7". Đóng bằng nút Đóng, bấm ra nền hay Esc; focus về lại xấp bìa lớn. Không có dòng chữ hướng dẫn (chủ dự án bỏ ngày 26/09). Trình xem **không bao giờ cuộn**: nó cắt (`overflow: clip`) thẻ vừa lật ở mép màn hình, chồng thẻ co theo chiều cao màn hình để luôn vừa, và ở màn hẹp dòng lượt luôn giữ chỗ hai dòng, nên chồng thẻ đứng yên khi lật. Thẻ 800ms (`--dur-the-bia`), nền 600ms (`--dur-nen-xem`); giảm chuyển động thì chỉ mờ chồng 150ms. Là `<dialog open>` không modal: phần còn lại của trang bị `inert` (trừ thẻ nhạc) và khóa cuộn, thanh cuộn biến mất được bù bằng `padding-right`.
...
- **Khi nào nhạc tự phát**: chỉ khi bấm một ngày có nhạc, phát tuần tự, hết bài sang bài kế, bài hỏng thì bỏ qua. Bấm lại đúng ngày đang phát thì phát tiếp. Xem bìa và đổi tháng không đụng tới nhạc. Chỉ bấm ngày khác mới dừng bài cũ và đổi (ngày không có nhạc thì dừng, và không còn trình phát nào). Vừa mở trang thì không phát gì. Một trình phát cho cả danh sách (`useMayPhatDanhSach`, đổi bài bằng `loadVideoById` ngay trong cú bấm).
- Khi trình xem bìa mở, thẻ nhạc **nổi trên lớp nền**: góc dưới phải ở màn rộng, giữa đáy màn hình (tối đa 26rem) từ 1000px trở xuống, và trình xem chừa đúng chiều cao của nó. Không lớp nào đè lên khung phát ở bất kỳ bề rộng nào; CSP không nới dòng nào.
- Tháng mặc định là tháng của dấu mới nhất; ngày chọn sẵn là ngày của dấu mới nhất trong tháng đó, tháng không có dấu thì như Lịch hoa (hôm nay, hay ngày cuối tháng).
