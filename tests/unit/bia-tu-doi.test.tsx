...
import { BiaTuDoi } from "@/components/book/BiaTuDoi";
...
describe("BiaTuDoi", () => {
  it("cuon mot bia: dung yen tuyet doi, khong mot hen gio nao duoc dat", () => {
    render(<BiaTuDoi covers={[bia("nui-xa")]} tuDatNut={false} />);
    choDoiBia();
    choDoiBia();
  ...
  it("cuon hai bia: sau muoi giay thi bia doi, va bia moi nam san o duoi", () => {
    render(<BiaTuDoi covers={[bia("nui-xa"), bia("hoa-dao")]} tuDatNut={false} />);
    expect(khung().className).toContain("bia--nui-xa");
    choDoiBia();
  ...
  it("go component thi khong con hen gio nao", () => {
    const { unmount } = render(<BiaTuDoi covers={[bia("nui-xa"), bia("hoa-dao")]} tuDatNut={false} />);
  ...
  it("tuDatNut bat: co nut tam dung, bam thi ghi dung lua chon dung chung", () => {
    render(<BiaTuDoi covers={[bia("nui-xa"), bia("hoa-dao")]} tuDatNut />);
    const nut = screen.getByRole("button", { name: "Tạm dừng hiệu ứng" });
    fireEvent.click(nut);
  ...
  it("tuDatNut tat: khong co nut nao", () => {
    render(<BiaTuDoi covers={[bia("nui-xa"), bia("hoa-dao")]} tuDatNut={false} />);
    expect(screen.queryByRole("button", { name: /hiệu ứng/ })).toBeNull();
  });

  it("cuon mot bia thi khong co nut tam dung du tuDatNut bat: khong co gi de dung", () => {
    render(<BiaTuDoi covers={[bia("nui-xa")]} tuDatNut />);
    expect(screen.queryByRole("button", { name: /hiệu ứng/ })).toBeNull();
  });

  it("ba bia: ba lan doi lien tiep khong lan nao lap lai bia vua hien", () => {
    render(<BiaTuDoi covers={[bia("nui-xa"), bia("hoa-dao"), bia("cau-go")]} tuDatNut={false} />);
  ...
  it("bia anh cua mot o hien ca tranh du phong lan anh", () => {
    render(<BiaTuDoi covers={[bia("nui-xa", "anh-1")]} tuDatNut={false} />);
    expect(khung().className).toContain("bia--nui-xa");
