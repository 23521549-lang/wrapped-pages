...
export async function publishDraft(
  ...
  const kept = trimTrailingBlank(sheets);
  ...
  return db.transaction(async (tx) => {
    ...
    // nam TRUOC lenh ghi dau tien (tx.insert(pages) o duoi), va moi thu chay truoc chung chi la lenh doc: SELECT ...
    ...
    const span = {
      actorId: ownerId, bookId: book.id, mode: book.mode, at: now,
      firstPosition: first, lastPosition: first + bound.length - 1,
    };
    await tx.insert(pages).values(bound.map((content, i) => ({ bookId: book.id, position: first + i, content, publishedAt: now })));
    if (seal) {
      const sealId = await insertSeal(tx, book.id, span.firstPosition, span.lastPosition, seal, sealTeaser(bound[0]));
      await recordActivity(tx, { ...span, kind: seal.kind === "trao-doi" ? "moi-trao-doi" : "dang-trang", sealId });
      if (seal.kind === "hen-gio") await recordActivity(tx, { ...span, kind: "mo-hen-gio", sealId, at: seal.opensAt });
    } else {
      await recordActivity(tx, { ...span, kind: "dang-trang", sealId: null });
    }
