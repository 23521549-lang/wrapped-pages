 ...
 */
export const khoangLuot = new QueryBuilder()
  .select({
...
export async function roundsOfBook(db: AnyDb, bookId: string): Promise<RoundSpan[]> {
  const rows = await db
    .select({ id: rounds.id, publishedAt: rounds.publishedAt, editedAt: rounds.editedAt, first: khoangLuot.first, last: khoangLuot.last })
    .from(rounds)
    .innerJoin(khoangLuot, eq(khoangLuot.roundId, rounds.id))
