import { checkImageSource, chooseEncoded, IMAGE_ENCODINGS, type ImageFailure } from "@/lib/media/image";
 ...
 * giai ma; trinh duyet khong giai ma duoc (HEIC tren Windows, tep hong, khong phai anh) thi khong doc duoc. Noi goi dong
 * bitmap khi xong.
 */
...
async function encodeFrom(canvas: HTMLCanvasElement, maxBytes: number, attempt: number): Promise<Blob | ImageFailure> {
  const { type, quality } = IMAGE_ENCODINGS[attempt];
...
/**
 * Ma hoa canvas theo IMAGE_ENCODINGS (WebP, roi JPEG), chooseEncoded quyet tung buoc; maxBytes la tran may chu nhan cua
 * loai. Lay byte xong thi tra bo dem cua canvas lai ngay (width = height = 0): mot canvas anh trong trang toi
 * 1200x1600 giu khoang 7.7 MB, ma Safari tren iOS khong thu hoi bo dem canvas som va co han cung cho moi trang - them
 * vai anh trong mot luot la du cham tran, va nguoi dung chi thay "anh lang le khong ma hoa duoc". Blob da tra ve doc
 * lap voi canvas, nen khong ai con can doc canvas sau khi ham nay xong.
 */
export async function encodeCanvas(canvas: HTMLCanvasElement, maxBytes: number): Promise<Blob | ImageFailure> {
  try {
    return await encodeFrom(canvas, maxBytes, 0);
  } finally {
    canvas.width = 0;
    canvas.height = 0;
  }
