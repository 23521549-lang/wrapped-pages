 ...
 *
 * Nhanh dai lon: bo chon neo vao `[data-k]` cua bau troi dang hien, bo qua trai troi cu, va bo qua mat dang bi cat ve 0
 * o che do o cua so.
 */
 ...
 *
 * Dai troi luon la mot the boc `.troi-dai`, ke ca khi chi mot nguoi giu tam trang, va cao DUNG bang noi dung nhu ban
 * mau da duyet (chu du an bo khuon giu cho ngay 26/09, spec bo sung B6): hai mat nam trong CUNG mot o luoi, nen dai cao
 * bang mat cao hon va doi cho hai troi khong xo dich gi. Thay mot tam trang bang bai tho hay loi nhan dai ngan khac thi
 * dai doi chieu cao mot lan, dung luc troi bat dau doi (trai troi cu nam ngoai dong chay, xem cuoi ham). Bam vao o cua
 * so thi hai troi doi cho bang mot vong song nuoc
 ...
 */
export function BauTroi({ tenKia, kia, minh: minhMay }: { tenKia: string; kia: TroiHien | null; minh: TroiHien | null }) {
  /*
   * Troi cua nguoi xem dang ve (spec bo sung B5): troi TAM vua tha (hop tha dat no ngay khi trang cuon toi day, khong cho
  ...
  return (
    <div className={caHai ? "troi-dai troi-cua-so" : "troi-dai"} ref={batDai}>
      ...
      {/* Trai troi cu cua lan loang. No nam NGOAI dong chay cua o luoi (.troi-dai > .troi--cu dat tuyet doi, phu kin
          dai), nen chieu cao dai di theo troi moi ngay tu khung hinh dau cua lan loang va khong co lai muon luc don.
