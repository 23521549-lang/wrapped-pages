...
/** Cac loai niem phong duoc chon, theo thu tu hien. Sach rieng tu chi con Khong va Hen gio. */
export function luaChon(partnerNickname: string | null): { kind: SealChoice; ten: string; ghi: string }[] {
...
type KindsProps = {
  ...
  value: SealChoice;
  onChange: (kind: SealChoice) => void;
  disabled: boolean;
};

/**
 * Cot chon loai niem phong: danh sach doc, moi dong mot radio, ten dam va mot dong mo ta. Chi soan du lieu;
 * kiem va gui do PublishBar lam.
 */
export function SealKinds({ partnerNickname, value, onChange, disabled }: KindsProps) {
  const id = useId();
  return (
    <fieldset className="loai-niem-ds" disabled={disabled} aria-describedby={`${id}-ghi`}>
      <legend className="d loai-niem-ds__t">Niêm phong</legend>
      <p className="loai-niem-ds__ghi" id={`${id}-ghi`}>
        {partnerNickname ? `Chọn cách ${partnerNickname} mở những trang này.` : "Chọn lúc bạn đọc lại được những trang này."}
      </p>
      <div className="loai-niem-ds__hang">
        {luaChon(partnerNickname).map((c) => (
          <label key={c.kind} className="loai-niem">
            <input type="radio" name={`${id}-loai`} value={c.kind} checked={value === c.kind} onChange={() => onChange(c.kind)} />
            <span className="loai-niem__t">{c.ten}</span>
            <span className="loai-niem__x">{c.ghi}</span>
          </label>
        ))}
      </div>
    </fieldset>
...
type FieldsProps = {
  ...
  disabled: boolean;
  /** min cua o datetime-local (gio dia phuong), PublishBar tinh mot lan luc mo. Chi goi y cho bo chon ngay, may chu van quyet. */
  minOpensAt: string;
...
/**
 * Cac o cua loai niem phong dang chon (cot giua). Loai Khong thi khong ve gi. Moi chu hien o day deu la nhan,
 * khong nhac dieu luat nao cua thu thach.
 */
export function SealFields({ value, onChange, disabled, minOpensAt, invalidAnswerIds }: FieldsProps) {
  const id = useId();
  ...
  if (value.kind === "trao-doi") return <div className="niem-o">{cauHoi}</div>;
  return null;
}
